...
import ua.testwork.racing.data.local.db.migration.SQLCipherUtils
import ua.testwork.racing.data.local.db.migration.SQLCipherUtils.encrypt
import ua.testwork.racing.data.local.db.migration.SQLCipherUtils.getDatabaseState
import ua.testwork.racing.data.local.db.migration.SqlCipherMigration
...
object MainModule {
    ...
    fun provideAppDatabase(app: Application): AppDatabase {
        SQLCipherUtils().run {
            val state = getDatabaseState(app, "racing.db")
            if (state.name == "UNENCRYPTED") {
                Log.i("DATABASE", "start encrypt")
                encrypt(app, "racing.db", "your-password".toByteArray())
            }
        }
